...
* Version 3.1.0
* Compatible haxe 3 - flash/js/openFL
* Usage :<br/>
...
class TweenObject {
	...
	public function new( target : Dynamic, properties : Dynamic, duration : Int, ?easing : Easing, autoStart = false, ?onFinish : Void->Void ) {
		...
		tweens		= #if haxe3 new GenericStack<TweenProperty>(); #else new FastList<TweenProperty>(); #end
		_nbTotal	= 0;
...
class Tween {
	...
	static function cb_tick( #if ( openfl || flash9 ) ?_ #end ) : Void	{
		for ( i in _aTweens )
			i.doInterval();
	}
